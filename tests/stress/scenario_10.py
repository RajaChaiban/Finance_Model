"""Stress test scenario #10 — Sovereign wealth XLRE post-rate-cut zero-cost protect_gains.

$60M XLRE, 12mo protect_gains, ~30bps near-zero budget, no barriers, capped upside OK.
Drives the structuring co-pilot end-to-end (Gates A → B → C) in DEMO_REPLAY mode,
then runs an independent Monte Carlo overlay against the recommended candidate
to verify QL prices for each leg.

Run: ``python tests/stress/scenario_10.py``
"""
...
# Windows cp1252 cannot encode Greek letters. Switch stdout to UTF-8.
try:
...
RFQ_TEXT = (
    "Sovereign wealth allocator, $60M XLRE post-rate-cut bounce. Want zero-cost "
    "protect_gains 12mo. Capped upside OK."
)
...
INTAKE_PAYLOAD: dict[str, Any] = {
    ...
    "constraints": ["no barriers", "rate-sensitive sector"],
...
def _simulate_paths(
    *,
    ...
    """Simulate GBM paths under risk-neutral measure (single-step terminal sufficient
    for European legs; we still include path-min for completeness).
    """
    dt = T / n_steps
...
def main() -> int:
    ...
    with _fake_market(SPOT, VOL_30D, VOL_90D, DIV_YIELD):
        ...
        for j, leg in enumerate(cand.legs):
            ...
            if leg.option_type.startswith("european_"):
                ...
                if B is None:
                    ...
                    breached = (mins <= B) if is_down else (ST >= B)
                    if bkind == "out":
        ...
        # ------------------------------------------------------------------
        # Net MC vs QL on the structure
        # ------------------------------------------------------------------
        # Compute the MC net premium across legs using same convention as PricingAgent:
        # USD = per_share * quantity, where quantity carries sign.
        mc_net_premium_usd = 0.0
        for o in leg_overlays:
            leg = o["leg"]
            if not math.isnan(o["mc"]):
                mc_net_premium_usd += o["mc"] * leg.quantity
        mc_net_bps = (mc_net_premium_usd / notional) * 10_000.0 if notional else float("nan")
        ql_net_bps = rec.net_premium_bps
        ql_net_usd = rec.net_premium
        net_delta_bps = mc_net_bps - ql_net_bps
        ...
        # zero_cost_only: |net_bps| < 5 is true zero cost; |net_bps| <= 30 is in budget.
        in_budget = abs(net_bps) <= BUDGET_BPS + 5
        true_zero_cost = abs(net_bps) < 5

        # Identify call cap relative to spot.
        call_leg = next(
        ...
        call_cap_pct = 100.0 * (call_leg.strike / S0 - 1.0) if call_leg else float("nan")
        put_strike_pct = 100.0 * (put_leg.strike / S0 - 1.0) if put_leg else float("nan")

        engine_ok = (worst_van <= 1.0) and (worst_bar <= 5.0)

        if engine_ok and in_budget and true_zero_cost:
            verdict = "PASS"
        elif engine_ok and in_budget:
            verdict = "WARN"
        else:
            verdict = "FAIL"
        ...
        print("=" * 80)
        print(
            "STRESS SCENARIO 10 — XLRE protect_gains $60M 12mo zero-cost capped-OK"
        )
        print("=" * 80)
        print(
            f"VERDICT: {verdict}   "
            ...
            f"in_budget={in_budget}  true_zero_cost={true_zero_cost}"
        )
        ...
        print()
        print(
            f"Put strike: K={put_leg.strike if put_leg else float('nan'):.2f} "
            f"({put_strike_pct:+.1f}% spot)   "
            f"Call cap:   K={call_leg.strike if call_leg else float('nan'):.2f} "
            f"({call_cap_pct:+.1f}% spot)"
        )
        print(
            f"Net premium (QL): ${ql_net_usd/1e6:,.3f}M  ({ql_net_bps:+.2f} bps)  "
            f"vs {BUDGET_BPS:.0f}bps budget — true_zero_cost={'Y' if true_zero_cost else 'N'}"
        )
        print(
            f"Net premium (MC): ${mc_net_premium_usd/1e6:,.3f}M  "
            f"({mc_net_bps:+.2f} bps)   net_delta_MC_vs_QL = {net_delta_bps:+.2f} bps"
        )
